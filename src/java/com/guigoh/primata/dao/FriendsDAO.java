...
public class FriendsDAO implements Serializable {
    ...
    public List<Friends> findFriendsByToken(String id) {
        EntityManager em = getEntityManager();
        try {
            List<Friends> friendList = (List<Friends>) em.createNativeQuery("select * from primata_friends "
    ...
    public List<SocialProfile> findFriendsOnlineByToken(String id) {
        EntityManager em = getEntityManager();
        try {
            List<SocialProfile> onlineList = (List<SocialProfile>) em.createNativeQuery("select sp.* from primata_messenger_status ms "
                    + "join primata_social_profile sp on ms.social_profile_id = sp.social_profile_id "
                    + "join primata_friends f on (sp.token_id = f.token_friend_2 or sp.token_id = f.token_friend_1) "
    ...
    public List<SocialProfile> findFriendsOfflineByToken(String id) {
        EntityManager em = getEntityManager();
        try {
            List<SocialProfile> offlineList = (List<SocialProfile>) em.createNativeQuery("select sp.* from primata_messenger_status ms "
                    + "join primata_social_profile sp on ms.social_profile_id = sp.social_profile_id "
                    + "join primata_friends f on (sp.token_id = f.token_friend_2 or sp.token_id = f.token_friend_1) "
                    + "where (f.token_friend_1 = '"+id+"' or f.token_friend_2 = '"+id+"') "
                    + "and sp.token_id <> '"+id+"' "
    ...
    public List<Friends> loadFriendSearchList(String token, String str) {
        EntityManager em = getEntityManager();
        try {
            List<Friends> friendsList = (List<Friends>) em.createNativeQuery("select distinct f.* from primata_friends f "
                    + "join primata_social_profile sp on ((f.token_friend_2 = sp.token_id) or (f.token_friend_1 = sp.token_id)) "
    ...
    public List<SocialProfile> loadUserSearchList(String str) {
        EntityManager em = getEntityManager();
        try {
            List<SocialProfile> usersList = (List<SocialProfile>) em.createNativeQuery("select * from primata_social_profile "
    ...
    public void updateRecommendQut(Friends friend) {
        EntityManager em = getEntityManager();
        try {
            String sql = "update primata_friends set recommenders = (recommenders+1) where "
