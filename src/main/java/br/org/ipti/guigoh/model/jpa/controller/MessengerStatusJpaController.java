 ...
 * @author IPTI
 */
public class MessengerStatusJpaController implements Serializable {

    private transient EntityManagerFactory emf = PersistenceUnit.getEMF();
    
    public MessengerStatusJpaController() {
    }
    ...
    public Long getUsersOnline(){
        EntityManager em = getEntityManager();
        try {
            Long usersOnline = (Long) em.createNativeQuery("select count(*) from messenger_status where (select round( date_part( 'epoch', now() ) )) - last_ping < 90").getSingleResult();
            return usersOnline;
        } finally {
            em.close();
        }
